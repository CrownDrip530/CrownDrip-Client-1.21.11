...
public class TargetESP extends HackModule {
    ...
    public void renderHud(DrawContext ctx) {
        ...
        Vec3d top    = new Vec3d(target.getX(), bb.maxY + 0.2, target.getZ());
        Vec3d bottom = new Vec3d(target.getX(), bb.minY - 0.2, target.getZ());
        Vec3d left   = new Vec3d(bb.minX - 0.1, target.getY() + target.getHeight()/2, target.getZ());
        Vec3d right  = new Vec3d(bb.maxX + 0.1, target.getY() + target.getHeight()/2, target.getZ());
        ...
        net.minecraft.text.Text info = net.minecraft.text.Text.literal(name + " ")
            .append(net.minecraft.text.Text.literal((int)hp + "/" + (int)maxHp)
                .styled(s -> s.withColor(hpColor)))
            .append(net.minecraft.text.Text.literal(" [" + dist + "m]")
